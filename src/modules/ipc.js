...
import { exportJSON } from "./flightlog.js"
...
export function subscribeListeners() {
  ...
  ipcMain.on("LOAD_FLIGHTLOG", (event) => {
    ...
    if (!paths) {
      event.sender.send("LOAD_FLIGHTLOG", { error: "No file selected" });
      return
    }
  ...
  ipcMain.on("EXPORT_FLIGHTLOG_JSON", (event, flightLog) => {
    exportJSON(flightLog)
    event.sender.send("EXPORT_FLIGHTLOG_JSON");
  });
